...
class BipMobileThemeOptions{
	...
	function & getAdminOptions($option_name=''){
		static $op_un_admin_options = null;
		if (!$op_un_admin_options){
			include (str_replace('//','/',dirname(__FILE__)).'/admin.options.php');
		}
		if ($option_name) return $op_un_admin_options[$option_name];
		return $op_un_admin_options;
	}
	function & getOptions($option_name='', $renew = false) {
		...
		//echo $i.'. getOptions ('.$options['op_un_normal_thumbnail_size_x'].') <br/>'; 
		if (!$options){
			//echo 'RENEW ';
			$options = get_option('op_un_options');
			$defaults = & self::getDefaults();
//if ($renew) {echo "!----";print_r($defaults);}
			if (is_array($defaults)){//is_admin()
				...
				update_option('op_un_options', $options);
			}
		...
		//echo 'end getOptions ('.$options['op_un_normal_thumbnail_size_x'].') <br/>'; 
	...
	function & getDefaults(){
		...
		if (!$default_values){
			$op_un_admin_options = & self::getAdminOptions();
	...
	function displaySingleOptionField($valueset,$admin_option,$sub_option_key){
		...
		switch ($valueset['input_type']){
			case 'select':
				if(isset($valueset['multiselect']) && $valueset['multiselect']){
					$selected = (($option)? explode(',',$option) : array());
					//var_dump($selected);
					echo '<select name="'.$option_name.'[]" multiple="multiple" size="10">';
					
					foreach ($valueset['values'] as $value => $diplay_text){
						echo '<option value="'.$value.'"'.((in_array($value, $selected))?'selected="selected"':'').'" '
			...
			case 'file':
				...
				echo '<div class="image" id="' . $option_name . '-div"><img src="'.get_template_directory_uri()."/assets/img/".((!is_null($option))?$option:$std).'" width="50" height="50"/>&nbsp;';
				echo '<input type="file" name="'.$option_name
					.'" value="" '.(($width)? "size=\"$width\"":'').'></div>';
				break;
			case 'checkbox':
				$width = (isset($admin_option['width'][$sub_option_key]))?$admin_option['width'][$sub_option_key]
						  :((!is_array($admin_option['width']))?$admin_option['width']:'');
	...
	function displaySingleAdminOption($option_name){
		...
		echo '<div class="'.self::cfg('style-prefix').'-admin-option">'; // start of a single admin option
			...
			if (isset ($admin_option['inherit'])){
				...
				echo '<option value="none">'.__('none',BIP_DOMAIN).'</option>';
	...
	function displayCategoryAdminOptions($category, $active){
		$admin_options = & self::getAdminOptions();
		$categories = & self::getOptionsCategories();
		echo '<div class="tab-pane '.self::cfg('style-prefix') . '-category-options '.$active.'" id="'.self::cfg('style-prefix') . '-category-'.str_replace(' ','_',$category).'-options">';
	...
	function displayAdminOptions(){
		$categories		= & self::getOptionsCategories();
		$options		= & self::getOptions('');
		$current_tab	= $options[self::cfg('shortname') . '_current_tab'];
		echo '<form action="#" method="post" enctype="multipart/form-data" name="'. self::cfg('shortname').'_form" style="padding-top:25px;">';
		//var_dump ($categories);
		echo '<div class="tabbable tabs-left '.self::cfg('style-prefix') . '-categories"><ul class="nav nav-tabs">';
		foreach ($categories as $category => $v){
			if (!isset($v['display_name'])) $v['display_name'] = $category;
			$id = self::cfg('style-prefix') . '-category-'.str_replace(' ','_',$category).'-button';
			echo '<li class="nav nav-tabs '.self::cfg('style-prefix').'-category-button '.(($current_tab==$id)?'active':'').' " id="'.$id.'"><a href="#'.self::cfg('style-prefix') . '-category-'.str_replace(' ','_',$category).'-options" data-toggle="tab">'
					. $v['display_name']
				. '</a></li>';
		}
		echo '</ul>';
		...
		foreach ($categories as $category => $v){
			...
			self::displayCategoryAdminOptions($category, $active);
		}
		
		echo '<div class="clear"></div><br/><br/>';
		echo '<input type="hidden" name="'. self::cfg('shortname') . '_current_tab'.'" id="'. self::cfg('shortname') . '_current_tab'.'"/><br/>';
		echo '<div id="save" class="row" style="margin-left:0px"><input type="submit" class="reset-options span2" value= "Reset ALL Options" name="'. self::cfg('shortname') . '_reset_options'.'"/>';
		echo '<input type="submit" value="Save options" class="save" style="margin-left:20px" name="'. self::cfg('shortname') . '_save_options'.'"/></div>';
		...
		jQuery(document).ready(function($) { 
			...
			var set_tab = "<?php echo ((isset($_POST[self::cfg('shortname').'_current_tab']))?$_POST[self::cfg('shortname').'_current_tab']:'') ?>";
			...
			$("."+prefix+"-category-button").click(function(){
				...
				$("#"+shortname+"_current_tab").val(buttonid);
	...
	function getSetOptionFile($option_name){
		$option_value = $option = & self::getOptions($option_name);			
		if($_FILES[$option_name]['error']!=0 || $_FILES[$option_name]['name']==''){
			
		}else{
			$fName			= $_FILES[$option_name]['name'];
			$type			= $_FILES[$option_name]['type'];
			$tmpName		= $_FILES[$option_name]['tmp_name'];
			$imagePath 		= get_template_directory()."/assets/img/";
			$imgExt 		= substr($fName, strrpos($fName,"."));
			$imageName 		= "custome_".substr($option_name, strrpos($option_name,"_")).$imgExt;
			if(copy($tmpName, $imagePath.$imageName)){
	...
	function getLayoutCSS(){
		...
		$page_templates = array(
			...
			'pg-tpl-no-sidebars-custom.php'=>'no_sidebars_custom',
		);
		$options = & self::getOptions();
		$shortname = self::cfg('shortname');
	...
	function echoOptionDescriptions(){
		...
		foreach ($admin_options as $id => $option){
			...
			echo "\t\t".'\'title\' => __(\''. addcslashes(stripslashes(($option_descriptions[$id]['title'])?$option_descriptions[$id]['title']:$option['title']), "'") .'\', BIP_DOMAIN),'."\n";
			echo "\t\t".'\'desc\' => __(\''. addcslashes(stripslashes(($option_descriptions[$id]['desc'])?$option_descriptions[$id]['desc']:$option['description']), "'") .'\', BIP_DOMAIN),'."\n";
			echo "\t),\n";
		}
		echo ');';
	}
	...
	function echoInlineCSS(){
		...
		ob_start();
		include_once( get_template_directory().'/options/inline.css.php');
	...
	function adminMenu(){
		...
		if(isset($_POST[$shortname . '_show_options_decription'])){//developer
		...
		if(isset($_POST[$shortname . '_save_options'])){
			...
			foreach($admin_options as $option_name => $admin_option){
				//print_r ($option_name);	print_r ($admin_option);die(); echo "<br><br>\n\n";
				...
				if (!is_array($admin_option['valueset'])){
					...
					if (isset($valuesets[$valueset_name]['multiselect']) && $valuesets[$valueset_name]['multiselect']){
						if($valuesets[$valueset_name]['input_type']=='select'){
							$option_value = isset($_POST[$option_name])?implode(',',$_POST[$option_name]):'';
							$options[$option_name] = 
									self::getOptionNewValue($option_name, $option_value,$valueset_name,$admin_option['std']);
						}else{						
						...
						if($valueset_name=='FILE'){
							$option_value = self::getSetOptionFile($option_name);
						}else{
					...
					foreach($admin_option['valueset'] as $sub_name=>$valueset_name){						
						if (isset($valuesets[$valueset_name]['multiselect']) && $valuesets[$valueset_name]['multiselect']){
							...
							if($valueset_name=='FILE'){
								$option_value = self::getSetOptionFile($option_name . '_' . $sub_name);
							}else{
		...
		add_menu_page( __( 'Bip Mobile Theme Options'), __( 'Bip Mobile' ), 'edit_themes', basename(__FILE__), array('BipMobileThemeOptions', 'displayAdminOptions'), WPTHEME_URL . '/assets/img/settings16x16.png');
		
	} //adminMenu

} //BipMobileThemeOptions