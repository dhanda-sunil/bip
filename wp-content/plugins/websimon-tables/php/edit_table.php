...
echo '<tr class="table-header"><td class="table-row-explan">Table Headlines<br />
	...
	<a href="javascript:void(0)"  onclick="document.getElementById(\'t' . $this_id . '_head_tr_attr\').style.display=\'block\'">
		<img src="' . get_bloginfo('wpurl') . '/wp-content/plugins/websimon-tables/images/add.png" title="Add custome attribute" />
	</a>
	<div style="display:none" id="t' . $this_id . '_head_tr_attr"><input type="text" name="t' . $this_id . '_head_tr_attr" value="'.stripslashes($thead_content[0]).'" /></div>
	</td>';
	while ($col_counter <= $numcol) { 
		echo '<td>
			<textarea rows="1" style="height: 30px;width:100%;" name="head' . $col_counter . '" />';
			if (isset($thead_content[$col_counter])) { echo stripslashes($thead_content[$col_counter]); }
			echo '</textarea>
		</td>';	
		$col_counter++;
	}
...
while ($row_counter <= $numrow) {
	$col_counter = 1;
	$cell = array();
	if (isset($tbody_content[$row_counter-1])) { $cell = explode('[-%cell%-]' , $tbody_content[$row_counter-1]); }
//explode each cell
	...
	<br />
	<a href="javascript:void(0)"  onclick="document.getElementById(\'t' . $this_id . '_body_tr'.$row_counter.'_attr\').style.display=\'block\'">
		<img src="' . get_bloginfo('wpurl') . '/wp-content/plugins/websimon-tables/images/add.png" title="Add custome attribute" />
	</a>
	<div style="display:none" id="t' . $this_id . '_body_tr'.$row_counter.'_attr"><input type="text" name="t' . $this_id . '_body_tr'.$row_counter.'_attr" value="'.((isset($cell[0]))?stripslashes($cell[0]):'').'" /></div>
	';
	echo '
	</td>';
	...
	while ($col_counter <= $numcol) {
		...
		if (isset($cell[$col_counter])) { echo stripslashes($cell[$col_counter]); }
		echo '</textarea></td>';
		$col_counter++;
		$cell_counter++;
